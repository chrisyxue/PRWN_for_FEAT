...
class FewShotModel(nn.Module):
        ...
        if self.training:
            if args.is_ssl == 1:
                return  (torch.Tensor(np.arange(args.way*args.shot)).long().view(1, args.shot, args.way), 
                     ...
                     torch.Tensor(np.arange(args.way*(args.shot + args.query), args.way * (args.shot + args.Nu +args.query))).long().view(1, args.Nu, args.way))
            else:
                return  (torch.Tensor(np.arange(args.way*args.shot)).long().view(1, args.shot, args.way), 
                     torch.Tensor(np.arange(args.way*args.shot, args.way * (args.shot + args.query))).long().view(1, args.query, args.way))
        else:
    ...
    def forward(self, x, get_feature=False):
        if get_feature:
            ...
            if self.args.is_ssl == 1 and self.training:
                support_idx, query_idx, unlabelled_idx = self.split_instances(x)
            else:    
                support_idx, query_idx = self.split_instances(x)

            if self.training:
                if self.args.is_ssl == 1:
                    proto_out, query_out, metric, logits, logits_reg  = self._forward(instance_embs, support_idx, query_idx,unlabelled_idx=unlabelled_idx)
                    return proto_out, query_out, metric, logits, logits_reg 
                else:
    ...
    def _forward(self, x, support_idx, query_idx,unlabelled_idx=None):
