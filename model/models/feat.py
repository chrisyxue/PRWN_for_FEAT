...
class FEAT(FewShotModel):
    ...
    def _forward(self, instance_embs, support_idx, query_idx,unlabelled_idx=None):
        ...
        num_batch = proto.shape[0] # Ntask
        num_proto = proto.shape[1] # the classes
        num_query = np.prod(query_idx.shape[-2:])
        if self.args.is_ssl == 1 and self.training:

            unlabelled = instance_embs[unlabelled_idx].contiguous().view(  *(unlabelled_idx.shape   + (-1,)))
            # unlabelled data -> [num_batch, Nu, num_proto, num_emb]
            unlabelled = unlabelled.permute([0,2,1,3]).reshape([num_batch,self.args.Nu*num_proto,-1])
    
        # query: (num_batch, num_query/num_proto, num_proto, num_emb)
        ...
        if self.args.use_euclidean:
            ...
            metric = "cos"
        proto_out = proto.clone()
        query_out = query.permute([0,2,1,3]).reshape([num_batch,num_query,-1]).clone() #[b,w*k,-1]
        ...
        if self.training:
            ...
            if self.args.is_ssl == 1:
                return proto_out, unlabelled, metric, logits, logits_reg          
            else:
